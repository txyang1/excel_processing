...
def update_excel(new_fp):
    ...
    clear_old = "Y" if src_cfg["pattern"] == "Octane" else "N"#自定义时请注释掉这一行,并将if clear_old == "Y":改成if clear_old：
    ...
    try:
        ...
        print("打开原表失败：原表正在被占用，请先关闭文件后重试")
        return 
    ...
    # 清除旧高亮
    if clear_old == "Y":   #自定义时请注释掉这一行,并将if clear_old == "Y":改成if clear_old:
    #if clear_old:
    ...
    wb.save(orig_fp)  
    time.sleep(5)
    print(f"[{ts}] 更新完成并保存到原表{orig_fp}, 请继续输入")
