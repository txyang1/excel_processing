...
def update_excel(new_fp):
    ...
    # 保存
    '''backup_dir = os.path.join(ORIG_DIR, "backups")
    os.makedirs(backup_dir, exist_ok=True)
    ts = datetime.now().strfttime("%Y%m%d_%H%M%S")
    backup_fp = os.path.join(backup_dir, f"summary_backup_{ts}.xlsx")
    shutil.copy2(orig_fp, backup_fp)
    print(f"已备份文件到：{backup_fp}")'''

    wb.save(orig_fp)
    print(f"[{ts}] 更新完成，保存至 {orig_fp}")
    #wb.save(upd_fp)
