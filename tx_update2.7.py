...
# -------------------------------
columns_to_remove = [
    # 如需要删除原表中的某些列，请列出名称（必须与原表中的完全匹配）
...
# -------------------------------
# 3. 定义“Found in function”到“Function”映射关系
# -------------------------------
# 当“Found in function”列中包含特定字符串时，在“Function”列中填写对应值
...
# -------------------------------
# 查找标题为 "Octane or Jira" 的列，若找到则对新追加的行（start_row 到 ws.max_row）填入对应值：
# 如果 new_file 文件名中含有 "Octane"，则填入 "Octane"；若含有 "Jira"，则填入 "Jira"。
...
print(f"更新完成！新数据已追加到工作表 '{target_sheet}' 的末尾，\n已根据 'Found in function' 更新了 Function 列，\n对新增行用黄色高亮显示，\n整个 'Days' 列均设置了公式, \n同时 'Octane or Jira' 列的新增区域已填入对应值。\n结果保存在 {updated_file}。")
