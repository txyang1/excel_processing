...
for _, new_row in df_new.iterrows():
    # 1) 找到 new_id
    new_id = new_row.get(
        next(k for k,v in mapping.items() if v=="ID"), "")
    ...
    if new_id in id2row:
        ...
        if "Found in function" in header2col and "Function" in header2col:
            ...
            for k,v in fund_map.items():
                if k in fv:
                    c=header2col["Function"]; ws.cell(last_row,c).value=v; ws.cell(last_row,c).fill=yellow
                    break
        if "Owner" in header2col and "Root cause" in header2col:
            ow=new_row.get("Owner","") or ""
            for k,v in own_map.items():
                if k in ow:
                    c=header2col["Root cause"]; ws.cell(last_row,c).value=v; ws.cell(last_row,c).fill=yellow
                    break

# === 5. 填公式 & 标记 Octane/Jira等 ===
max_row = ws.max_row
...
if nt and pl and ti:
    pL=get_column_letter(pl); tL=get_column_letter(ti)
    for r in range(2,max_row+1):
        ws.cell(r,nt).value=f'=IF(OR({pL}{r}<>"",{tL}{r}<>""),0,1)'

# === 6. 保存 ===
wb.save(upd_fp)
print(f"完成：来源={source_key}，结果写入 {upd_fp}")
