...
# -------------------------------
columns_to_remove = [
    # 如需要删除某些列，请在此列表中列出原表中列名（必须完全匹配）
    # 例如："SomeColumnName", "AnotherColumn"
]
columns_to_add = {
    # 如需增加新列，可用字典方式指定：键为列名称，值为默认值
    # 例如："新列1": "",
    #          "测试列": 0
}
...
# -------------------------------
# 当原表中“Found in function”（这里的名称与原表完全匹配）中包含指定的字符串时，
# 在“Function”列中填入对应的值
...
# -------------------------------
# 这里采用子串匹配，只要"Found in function"中包含 fund_function_mapping 中的键字符串，
# 则将对应的值填入“Function”列
...
# -------------------------------
# 10. 对新追加的行在“Days”列设置公式（继承原表公式）
# -------------------------------
# 假设原表头行中有一列名为 "Days"，且公式需要根据行号生成，且日期数据位于J列（请根据实际情况调整）
days_col_index = None
for col in range(1, ws.max_column + 1):
...
# 如果找到 Days 列，则对新追加的每一行设置公式。公式模板示例：=DATEDIF($J{row},TODAY(),"D")
...
print(f"更新完成！新数据已追加到工作表 '{target_sheet}' 的末尾，\n根据'Found in function'更新了Function列，对新增行用黄色高亮显示，\n并在 'Days' 列设置了公式。\n结果保存在 {updated_file}。")
