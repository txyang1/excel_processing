...
for _, new_row in df_new.iterrows():
    ...
    if new_id in id2row:
        ...
        if any(keyword in phase_val for keyword in ("Concluded","Closed","Resolved")):
            continue
        ...
        for nk, ok in mapping.items():
            ...
            if pd.isna(val) or val=="":
                continue
            if ok=="Invoved I-Step:" and (str(val).startswith("G070") or str(val).startswith("U006")):
                val = "NA05"+str(val)[4:]
            c = header2col.get(ok)
            if c:
                cell = ws.cell(r,c)
        ...
        # —— 更新 Function 列 via fund_patterns —— 
        if "Found in function" in header2col and "Function" in header2col:
            fv = (ws.cell(r, header2col["Found in function"]).value or "").lower()
        ...
        # —— 更新 Root cause 列 via owner_patterns —— 
        if "Owner" in header2col and "Root cause" in header2col:
            ow = (ws.cell(r, header2col["Owner"]).value or "").lower()
        ...
        # 新增 Function via fund_patterns
        if "Found in function" in header2col and "Function" in header2col:
            fv = (new_row.get("Found in function","") or "").lower()
            for func_name, kws in fund_patterns.items():
                if any(kw.lower() in fv for kw in kws):
        ...
        # 新增 Root cause via owner_patterns
        if "Owner" in header2col and "Root cause" in header2col:
            ow = (new_row.get("Owner","") or "").lower()
            for cause, names in owner_patterns.items():
                if any(n.lower() in ow for n in names):
...
# === 6. 保存结果 ===
wb.save(upd_fp)
print(f"完成 (来源={source_key})，新增行粉色，高亮填充；更新行淡蓝，高亮填充。结果保存在 '{upd_fp}'。")
